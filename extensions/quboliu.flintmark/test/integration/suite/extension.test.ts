...
describe("OFM custom editor (integration)", () => {
  ...
  it("ofm.setAsDefaultEditor registers the markdown editor association", async () => {
    ...
    try {
      ...
      await delay(600);
      for (const glob of ["*.md", "*.mkd", "*.mkdn", "*.mdwn", "*.mdown", "*.markdown", "*.markdn", "*.mdtxt", "*.mdtext", "*.litcoffee", "*.ron", "*.ronn", "*.workbook", "**/.cursor/**/*.mdc"]) {
        assert.strictEqual(cfg()[glob], VIEW_TYPE, `${glob} should map to our editor`);
      }
    } finally {
