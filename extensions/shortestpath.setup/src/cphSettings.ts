...
// Opens the Competitive Champion integration guide in the Integrated Browser
// from the top of the CPH settings page.
const competitiveChampionDocUrl = 'https://kevinhuang.feishu.cn/wiki/BsCxw3rVBi7ydmkmjMRcphbNnje';

async function openCompetitiveChampionDoc(): Promise<void> {
	try {
		await vscode.window.openBrowserTab(competitiveChampionDocUrl, { viewColumn: vscode.ViewColumn.Active, preserveFocus: false });
	} catch (error) {
...
function openCphSettings(): void {
	...
	panel.webview.onDidReceiveMessage(async message => {
		...
		if (message?.type === 'competitiveChampionDoc') { await openCompetitiveChampionDoc(); }
	});
