...
suite('Portable ShortestPath toolchain paths', () => {
	...
	test('finds the common portable data root for default and named profiles', () => {
		assert.deepStrictEqual([
			getPortableDataRoot('F:\\Shortest Path\\data\\user-data\\User\\globalStorage\\shortestpath.shortestpath-setup'),
			getPortableDataRoot('F:\\Shortest Path\\data\\user-data\\User\\profiles\\abc\\globalStorage\\shortestpath.shortestpath-setup'),
			getPortableDataRoot('C:\\Users\\me\\AppData\\Roaming\\ShortestPath\\User\\globalStorage')
		], ['f:/shortest path/data', 'f:/shortest path/data', undefined]);
	});

	test('rebases only managed query-driver arguments', () => {
		assert.deepStrictEqual([
			rebaseManagedQueryDriver('--background-index', compiler),
			rebaseManagedQueryDriver('--query-driver=C:/Custom/g++.exe, E:/Old/User/globalStorage/shortestpath.shortestpath-setup/toolchains/winlibs/bin/g++.exe', compiler),
			rebaseManagedQueryDriver('--query-driver=C:/MySDK/toolchains/winlibs/bin/g++.exe', compiler),
			...
			'--query-driver=C:/MySDK/toolchains/winlibs/bin/g++.exe',
			'--query-driver=C:/MySDK/toolchains/winlibs/mingw64/bin/g++.exe'
		]);
	});

	test('rewrites only ShortestPath-generated clangd configuration', () => {
		const oldCompiler = 'E:/Old/User/globalStorage/shortestpath.shortestpath-setup/toolchains/winlibs/bin/g++.exe';
