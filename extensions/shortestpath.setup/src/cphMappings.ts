...
type Field = { key: string; label: string; hint?: string; options?: readonly { value: string; label: string }[] };
...
const ojFields: Field[] = [
	...
	{ key: 'problemSource', label: localize('题面来源'), hint: localize('留空时使用 CPH 设置中的默认题面来源。'), options: [{ value: '', label: localize('使用默认值') }, { value: 'vjudge', label: 'VJudge' }, { value: 'original', label: localize('原 OJ') }] }
];
const vjudgeFields: Field[] = [
	...
	{ key: 'problemIdRegex', label: localize('题号正则') }, { key: 'compositeFormat', label: localize('组合题号格式') }, { key: 'vjudgeUrlKey', label: localize('VJudge URL 名称覆盖') }
];

export function openOjMappings(): void { open(localize('在线评测映射'), 'ojMapping', ojFields); }
...
function open(title: string, setting: 'ojMapping' | 'vjudgeOjNames', fields: readonly Field[]): void {
	...
	panel.webview.html = localizeWebviewHtml(html(title, fields, entries));
	panel.webview.onDidReceiveMessage(async message => {
		...
		try {
			...
			for (const input of message.entries as Record<string, unknown>[]) {
				...
				if (!name) { throw new Error(localizeFormat('“{0}”不能为空。', fields[0].label)); }
				if (result[name]) { throw new Error(localizeFormat('“{0}”重复。', name)); }
...
function html(title: string, fields: readonly Field[], entries: readonly Record<string, string>[]): string {
	...
	return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline';"><style>body{background:var(--vscode-editor-background);color:var(--vscode-foreground);font-family:var(--vscode-font-family)}main{max-width:1050px;margin:auto;padding:34px 28px}.entry{border:1px solid var(--vscode-editorWidget-border);border-radius:6px;padding:12px;margin:12px 0}.row{display:grid;grid-template-columns:210px 1fr;gap:12px;align-items:center;padding:6px 0}.hint{font-size:12px;color:var(--vscode-descriptionForeground)}input,select{width:100%;box-sizing:border-box;padding:7px;background:var(--vscode-input-background);color:var(--vscode-input-foreground);border:1px solid var(--vscode-input-border)}button{padding:7px 12px;border:0;margin-right:8px;background:var(--vscode-button-background);color:var(--vscode-button-foreground)}.danger{background:var(--vscode-button-secondaryBackground);color:var(--vscode-button-secondaryForeground)}#error{color:var(--vscode-errorForeground)}@media(max-width:650px){.row{grid-template-columns:1fr}}</style></head><body><main><h1>${title}</h1><p>以条目方式配置；留空的可选字段不会写入。正则请使用捕获组表示比赛和题目 ID。</p><div id="entries"></div><button id="add">新增映射</button><button id="save">保存</button><span id="status"></span><p id="error"></p></main><script>const vscode=acquireVsCodeApi(),fields=${fieldData},entries=${initial},root=document.getElementById('entries'),esc=s=>String(s??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;'),save=()=>vscode.postMessage({type:'save',entries});function control(field,value){if(!field.options)return '<input data-key="'+esc(field.key)+'" value="'+esc(value)+'">';return '<select data-key="'+esc(field.key)+'">'+field.options.map(option=>'<option value="'+esc(option.value)+'"'+(option.value===value?' selected':'')+'>'+esc(option.label)+'</option>').join('')+'</select>'}function render(){root.innerHTML='';entries.forEach((entry,index)=>{const card=document.createElement('div');card.className='entry';card.innerHTML=fields.map(field=>'<div class="row"><div><b>'+esc(field.label)+'</b>'+(field.hint?'<div class="hint">'+esc(field.hint)+'</div>':'')+'</div>'+control(field,entry[field.key])+'</div>').join('')+'<button class="danger">删除此映射</button>';card.querySelectorAll('input,select').forEach(input=>{const update=()=>entry[input.dataset.key]=input.value;input.oninput=update;input.onchange=()=>{update();if(input.dataset.key==='problemSource')save()}});card.querySelector('button').onclick=()=>{entries.splice(index,1);render()};root.append(card)})}document.getElementById('add').onclick=()=>{entries.push({});render()};document.getElementById('save').onclick=save;window.onmessage=e=>{if(e.data?.type==='saved'){document.getElementById('status').textContent='已保存';document.getElementById('error').textContent=''}if(e.data?.type==='error')document.getElementById('error').textContent=e.data.message};render();</script></body></html>`;
}
