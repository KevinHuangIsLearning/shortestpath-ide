...
const apiOrigin = (process.env.ATOMGIT_API_URL || 'https://api.gitcode.com').replace(/\/$/, '');
const apiBaseUrl = apiOrigin.endsWith('/api/v5') ? apiOrigin : `${apiOrigin}/api/v5`;
...
const token = process.env.GITCODE_TOKEN || process.env.ATOMGIT_TOKEN;
if (!token) {
	throw new Error('Missing required environment variable: GITCODE_TOKEN or ATOMGIT_TOKEN');
}
...
const targetCommitish = (await readFile(commitFile, 'utf8')).trim();
...
if (!/^[0-9a-f]{40}$/i.test(targetCommitish)) {
	throw new Error('GitHub tag commit is not a full commit SHA.');
}

let gitCodeTag;
for (let page = 1; !gitCodeTag && page <= 100; page++) {
	const tagsResponse = await request(`${repositoryPath}/tags?per_page=100&page=${page}`);
	if (!tagsResponse.ok) {
		throw new Error(`GitCode tag lookup failed with HTTP ${tagsResponse.status}.`);
	}
	const tags = await tagsResponse.json();
	if (!Array.isArray(tags)) {
		throw new Error('GitCode tag lookup returned invalid data.');
	}
	gitCodeTag = tags.find(candidate => candidate.name === tag);
	if (gitCodeTag || tags.length < 100) {
		break;
	}
}
if (!gitCodeTag) {
	throw new Error(`GitCode tag ${tag} does not exist. Push the tag before publishing its Release.`);
}
if (gitCodeTag.commit?.sha !== targetCommitish) {
	throw new Error(`GitCode tag ${tag} points to ${gitCodeTag.commit?.sha ?? 'an unknown commit'}, expected ${targetCommitish}.`);
}
...
if (!Array.isArray(existingAssets)) {
	throw new Error('GitCode Release assets metadata is invalid.');
}
for (const asset of existingAssets) {
	if (!asset.id || !asset.name) {
		throw new Error('GitCode Release asset metadata is missing an id or name.');
	}
	console.log(`Removing existing GitCode asset ${asset.name}.`);
	const deleteResponse = await request(`${releasePath}/${encodeURIComponent(tag)}/attach_files/${encodeURIComponent(asset.id)}`, {
		method: 'DELETE',
	});
	if (!deleteResponse.ok) {
		throw new Error(`Deleting existing GitCode asset ${asset.name} failed with HTTP ${deleteResponse.status}.`);
	}
}
...
console.log(`Uploading ${assetsToUpload.length} asset(s) with up to ${maxConcurrentUploads} concurrent uploads.`);
