...
class ProblemMatcherRegistryImpl implements IProblemMatcherRegistry {
	...
	private fillDefaults(): void {
		...
		// GCC/Clang diagnostic matcher. Contributed upstream by the cpptools extension
		// (name "gcc", owner "cpptools"); built in here so tasks referencing `$gcc` work
		// without cpptools installed (e.g. the OI distribution ships clangd instead).
		this.add({
			name: 'gcc',
